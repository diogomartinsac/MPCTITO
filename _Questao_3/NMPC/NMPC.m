...
nsim = 1500       ;    % Número de Simulações
...
dumax = 0.1/10*3*ones(Hp,1); dumin = -dumax;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    ...
    du = fmincon(@(du) fob_MPC(xmk,du,uk_1,Hp,Hc,sysi1,ysp,q,r,Q),du0,Ain,Bin(uk_1),Aeq,beq,dumin,dumax);
  % du = fmincon(@(du) fob_MPC(xmk,du,uk_1,Hp,Hc,sysi1,ysp,q,r,Q),du0,Ain,Bin(uk_1),Aeq,beq,dumin,dumax);  
    uk(:,k) = uk_1 + du(1:nu);
    uk_1 = uk(:,k);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    % MPC Dinâmica 2
      %options = optimoptions('patternsearch','UseParallel', true, 'UseCompletePoll', true, 'UseVectorized', false);
    du = fmincon(@(du) fob_MPC(xmk,du,uk_1,Hp,Hc,sysi2,ysp,q,r,Q),du0,Ain,Bin(uk_1),Aeq,beq,dumin,dumax);
