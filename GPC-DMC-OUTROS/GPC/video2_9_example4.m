...
ref = [zeros(1,5),0.5*ones(1,25)];
dist=[zeros(1,20),0.2*ones(1,10)];
...
subplot(221);plot(time,y','b-',time,yt','r:',time,r','m--');ylim([0,0.8])
xlabel(['Outputs and set-point']);
ss=legend('GPC','GPCT');set(ss,'fontsize',20);
subplot(222);plot(time,Du','b-',time,Dut','r:');ylim([-0.25,0.2])
xlabel(['Input increments']);
subplot(223);plot(time,u','b-',time,ut','r:');ylim([0 0.2])
xlabel(['Inputs']);
