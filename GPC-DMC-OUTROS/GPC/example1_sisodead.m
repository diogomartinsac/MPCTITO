...
catch  %%%% quadprog not available so cannot do constraints properly
disp(' ');
disp(' ');
disp('Quadprog not available - reverting to unconstrained');
disp(' ');
%[y,u,Du,r] = mpc_simulateb(B,A,nu,ny,Wu,Wy,Dumax,umax,umin,ref,dist,noise);
%[yt,ut,Dut,r] = mpc_simulate_tfiltb(B,A,Tfilt,nu,ny,Wu,Wy,Dumax,umax,umin,ref,dist,noise);
end 
